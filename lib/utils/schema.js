...
export const encodeAtomicizedSell = (schemas, assets, address, wyvernProtocol, networkName) => {
...
export const encodeAtomicizedBuy = (schemas, assets, address, wyvernProtocol, networkName) => {
    const atomicizer = wyvernProtocol.wyvernAtomicizer;
...
export const encodeBuy = (schema, asset, address) => {
    const transfer = schema.functions.transfer(asset);
    const replaceables = transfer.inputs.filter((i) => i.kind === FunctionInputKind.Replaceable);
    const ownerInputs = transfer.inputs.filter((i) => i.kind === FunctionInputKind.Owner);
    // Validate
 ...
 */
export function encodeProxyCall(address, howToCall, calldata, shouldAssert = true) {
